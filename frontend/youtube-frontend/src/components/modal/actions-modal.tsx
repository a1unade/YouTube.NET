...
const ActionsModal = (props: {
  ...
  const { active, setActive, setSaveActive, setReportVideoActive, buttonRef } = props;
    ...
    <>
      {active && (
        <div ref={modalRef} className="actions-modal-window">
          <div>
            <button style={{ paddingRight: 20 }} onClick={openSaveModal}>
              <AddToPlaylistIcon />
              <span>Сохранить</span>
            </button>
          </div>
          <div>
            <button style={{ paddingRight: 20 }} onClick={openReportModal}>
              <FlagIcon />
